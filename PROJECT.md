...
## 七、锚点容错 + 形状栈修复 + 页码回补（2026-07-26）

《中国当代社会各阶层分析》（414 页扫描本，章扉页竖排标题、节标题无编号）
切片实测暴露三连崩，均在 stage2_common.py 修复：

1. **章标题整块丢失**（MinerU 对竖排章扉页整块漏识别，第一章章名回退成
   书名）→ 新增 `_rescue_by_page`：toc_entries 增加 page 字段（轻量兜底
   prompt 同步改），按页码定位救援未锚上的目录条目。
2. **节被提升成章**（形状栈真 bug）：章名块与节标题同为 plain 形状时
   同键碰撞，兄弟节弹栈把父章一并弹出、空栈兜底取到被污染的
   last_shape_level → 锚定条目压栈键改用真实层级（float(toc_level)）。
3. **锚点一字之差失配**（目录页 OCR 僵→催、目录与正文措辞 是/和、缺
   "的"）→ `_match_anchor` 在精确/前缀/后缀后加两级兜底：子串匹配
   （限 ≥8 字符，救 OCR 截断的副标题）+ 有界编辑距离（键长 <12 容忍
   1 字，≥12 容忍 2 字，等距并列放弃防误配）。

**全量验证暴露的第二轮问题（同日修复）**：

- **扫描件丢印刷页 → 偏移漂移**：该书 PDF−印刷页偏移前段 +15、中段
  +14、后段 +12（章末空白页未被扫描），全局众数必然错位 → 偏移取
  **局部值**（印刷页码最近邻锚点的偏移），全局众数仅用于剔除野票
  （±5 以外，如封面残片误锚产生的负数票）。
- **封面残片毒化 matched_keys**：封面印的书摘句（"城乡过渡阶层——
  流动民工"等）后缀匹配了第九/十四章锚点，导致这两章回补被抑制、
  内容折进别章 → "条目已满足"判定加位置 sanity：匹配块页码须落在
  条目预期页 ±3 内。
- **短标题超出文本模糊阈**（目录页 OCR 卖淫女→奕淫女，3 字标题）→
  页码+模糊联合证据晋升：候选页上找编辑距离 ≤limit 的块晋升标题
  （节级只晋升不合成；章级找不到近似块才按目录文本合成，且只插
  块数 ≤4 的稀疏页——稠密页宁可不救，防标题插进上一章末尾）。
- **章名缺"第 X 章"前缀 / 副标题截断**：锚定命中（前缀/尾部/子串）
  且块文是锚点子串时，块内容富化为目录完整文字（原"裸标题富化"
  从渲染期前移到校准期，分隔页块与副标题块富化后互为包含仍可合并）。

**验证**：全书 414 页（Hybrid 50s）：18/18 章单元、章名全带"第 X 章"
前缀、节目录与原书逐项一致（含救回的卖淫女节）；回补 9 章（一/五/
六/九/十一/十四/十五/十六/十八）全部落位正确。回归《城市与国家财富》
《民法总论》离线重校准：层级零变化，标题文本仅按既有"裸标题富化"
语义补全。**残留**：第十三章名缺"买办"字（MinerU 在目录页与正文页
同处漏识，锚点与块同错，管线内无真值可校）；个别正文 OCR 错字标题
（"农民收人"）层级正确但按原样渲染。页眉捞标题的思路评估后放弃
（重复文本+OCR 噪声，不稳）。
