...
| Key | 获取 | 用途 |
|---|---|---|
| MinerU Token | https://mineru.net/apiManage/token | OCR + 版面分析（免费） |
...
2. **设置**：
   - MinerU Token / LLM Base URL / LLM API Key / LLM Model
   - 强制 OCR：扫描书必开；出版社出的**文字版 PDF 关掉更快**
     （关掉后 MinerU 直接抽文字层，不做图像识别，速度翻倍）
   - 翻译为 [语言]：勾选后全书翻译为目标语言
   - 界面语言：中文 / English
3. **开始转换**：壹·MinerU → 贰·结构重建 →（叁·翻译）→ 肆·EPUB，
   进度条按实测速率校准
...
```bash
python pipeline.py book.pdf                  # 标准转换
...
```
app.py               # GUI 前端（拖拽多书/密钥设置/队列进度/中英双语）
启动.bat             # 一键启动 GUI
pipeline.py          # 命令行主流程
